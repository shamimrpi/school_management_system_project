    ...
    public function getSalary(Request $r){
      ...
      $date = date('Y-m',strtotime($r->date));
       ...
       foreach ($data as $key => $attend) {
           ...
           $totalSalaryMinus = $absentCount*(float)$salaryPerday;
           $totalSalary      = (float)$salary-(float)$totalSalaryMinus;

           $html[$key]['tdsource'] .= '<td>'.$totalSalary.'TK'.'</td>';
           $html[$key]['tdsource'] .= '<td>';
           $html[$key]['tdsource'] .= '<a class="btn btn-sm btn-success" title="Salary Pay Slip" target="_blank" href="'.route("salary.payslip",$attend->employee_id).'">Salary PaySlip</a>';
           $html[$key]['tdsource'] .= '</td>';
       }
